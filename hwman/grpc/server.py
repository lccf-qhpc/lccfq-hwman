...
class JobService(JobDispatchServicer):
    def SubmitJob(self, request, context):
        ...
        logger.info(
            f"Authenticated user '{user_id}' submitted job: {request.id}, type: {request.type}"
        )
        ...
        if request.type == JobType.JOB_TYPE_DUMMY:
            ...
            ret = Job(
                ...
                user=request.user
                or user_id,  # Use cert user if not provided in request
                type=request.type,
...
def create_mtls_server(
    port: str = "50051", cert_dir: Path = Path("./certs"), hostname: str = "localhost"
):
    """
    ...
    if clients:
        logger.info("Existing client certificates:")
...
def serve(port: str = "50051", cert_dir: str = "./certs", hostname: str = "localhost"):
    """Start the mTLS gRPC server."""

    server, cert_manager = create_mtls_server(
        port=port, cert_dir=Path(cert_dir), hostname=hostname
    )
...
if __name__ == "__main__":
    ...
    serve(port="50051", cert_dir="./certs", hostname="localhost")
