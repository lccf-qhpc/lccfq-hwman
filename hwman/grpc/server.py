...
from hwman.grpc.protobufs_compiled import jobs_pb2_grpc
from hwman.grpc.protobufs_compiled.jobs_pb2 import JobType, JobStatus, Result, DataAxis, Job
from hwman.grpc.protobufs_compiled.jobs_pb2_grpc import JobDispatchServicer
...
def dummy_measurement():
    ...
    for dep in data.dependents():
        axis = DataAxis(name=dep, values=list(int(x) for x in data[dep]["values"]), depends_on=data[dep]["axes"])
        axes.append(axis)
    result = Result(id=str(uuid.uuid4()), path=str(data_location), data=axes)
    return result


class JobDispatchServicer(JobDispatchServicer):
...
def serve():
    ...
    jobs_pb2_grpc.add_JobDispatchServicer_to_server(JobDispatchServicer(), server)
