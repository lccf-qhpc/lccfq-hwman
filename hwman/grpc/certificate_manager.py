...
class CertificateManager:
    ...
    def _create_ca_certificate(self) -> Tuple[x509.Certificate, rsa.RSAPrivateKey]:
        ...
        ca_cert = x509.CertificateBuilder().subject_name(
            ...
            datetime.now(timezone.utc) + timedelta(days=3650)  # CA valid for 10 years
        ).add_extension(
    ...
    def _create_server_certificate(
        ...
        server_cert = x509.CertificateBuilder().subject_name(
            ...
            datetime.now(timezone.utc) + timedelta(days=365)  # Server cert valid for 1 year
        ).add_extension(
            ...
            critical=True,
        ).sign(ca_private_key, hashes.SHA256())  # Signed by CA's private key
    ...
    def _create_client_certificate(
        ...
        client_cert = x509.CertificateBuilder().subject_name(
            ...
            datetime.now(timezone.utc) + timedelta(days=365)  # Client cert valid for 1 year
        ).add_extension(
            # This is NOT a CA certificate
            x509.BasicConstraints(ca=False, path_length=None),
            critical=True,
    ...
    def setup_ca_and_server(self, hostname: str = "localhost") -> Tuple[Path, Path, Path]:
        """
        ...
        if not (self.server_cert_file.exists() and self.server_key_file.exists()):
            ...
            self._save_certificate_and_key(
                server_cert, server_private_key, self.server_cert_file, self.server_key_file
            )
        else:
