...
def setup_server_certificates(cert_dir: str, hostname: str):
    ...
    print("2. Create client certificates: python -m hwman.grpc.certificate_cli create-client <user_id>")
...
def create_client_certificate(user_id: str, cert_dir: str):
    ...
    if not (cert_manager.ca_cert_file.exists() and cert_manager.server_cert_file.exists()):
        ...
        print("Run setup-server first: python -m hwman.grpc.certificate_cli setup-server")
        return False
...
def list_client_certificates(cert_dir: str):
    ...
    if not clients:
        ...
        print("Create one with: python -m hwman.grpc.certificate_cli create-client <user_id>")
        return
...
Examples:
  ...
  %(prog)s status                           # Show server certificate status
    ...
    setup_parser = subparsers.add_parser(
        'setup-server', 
        help='Set up CA and server certificates'
    )
    ...
    create_parser = subparsers.add_parser(
        'create-client',
        help='Create a client certificate for a user'
    )
    create_parser.add_argument(
        'user_id',
        help='User ID for the certificate'
    )
    
    # List clients command
    subparsers.add_parser(
        'list-clients',
        help='List all existing client certificates'
    )
    
    # Status command
    subparsers.add_parser(
        'status',
        help='Show server certificate status'
    )
