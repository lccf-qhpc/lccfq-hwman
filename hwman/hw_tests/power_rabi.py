...
from qcui_measurement.qick.single_transmon_v2 import AmplitudeRabiProgram

from hwman.utils.fitting import FitSpec, fit_in_subprocess, serialize_params

logger = logging.getLogger(__name__)

FAKEDATA = Path("test_data/2025-09-15T220655_811849a0-Amplitude rabi~782ed6ce")


def measure_power_rabi(job_id: str):

    logger.info("Starting power rabi for {}".format(job_id))

    sweep = AmplitudeRabiProgram()
    logger.debug("Sweep created, running measurement")
    loc, da = run_and_save_sweep(sweep, "data", f"power_rabi~{job_id}", return_data=True)
    logger.info("Measurement done, data in %s", loc)
    return loc, da


# TODO: multiply the SNR by 2 since the amplitude is from the middle not peak to peak.
...
def plot_power_rabi(x: np.ndarray, y: np.ndarray, fit_result, plot_path: Path, plot_type: str):
    fit_curve = fit_result.eval()

    plot_spec = PlotSpec(
        plot_path=str(plot_path),
        title=f"Power Rabi ({plot_type})",
        xlabel="Gain (A.U)",
...
def analyze_power_rabi(loc: Path):

    logger.info("Starting to analyze power rabi for {}".format(loc))

    if not loc.exists():
        msg = f"Power rabi for {loc} does not exist"
        raise FileNotFoundError(msg)

    data = datadict_from_hdf5(loc/"data.ddh5")

    with DatasetAnalysis(loc, "power_rabi") as ds:
    ...
    for f in savefolders:
        plot_power_rabi(x=gains, y=signal_re, fit_result=fit_result_re, plot_path=f / "real_fit.png" , plot_type="Real")
        plot_power_rabi(x=gains, y=signal_imag, fit_result=fit_result_imag, plot_path=f / "imag_fit.png" , plot_type="Imaginary")
        plot_power_rabi(x=gains, y=signal_mag, fit_result=fit_result_mag, plot_path=f / "mag_fit.png", plot_type="Magnitude")

    return result_re, result_imag, result_mag

def power_rabi(job_id: str, fake_calibration_data: bool = False):
    loc, da = measure_power_rabi(job_id)
    ...
    ret_real, ret_imag, ret_mag = analyze_power_rabi(loc)
