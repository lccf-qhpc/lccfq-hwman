...
from qcui_measurement.qick.single_transmon_v2 import T2RProgram, T2nProgram

from hwman.utils.fitting import FitSpec, fit_in_subprocess, serialize_params

logger = logging.getLogger(__name__)

FAKEDATAR = Path("test_data/2025-09-15T220660_e122324a-T2R~87237761")
FAKEDATAE = Path("test_data/2025-09-15T220661_32f09440-T2E~62e1d951")


def measure_t2x(job_id: str, n_echos: int):

    logger.info("Starting T2* measurement for {}".format(job_id))

    sweep = T2RProgram()
    if n_echos>0:
        sweep = T2nProgram()
    logger.debug("Sweep created, running measurement")
    loc, da = run_and_save_sweep(sweep, "data", f"t2x~{job_id}", return_data=True)
...
def plot_t2x(x: np.ndarray, y: np.ndarray, fit_result, plot_path: Path, plot_type: str):
    fit_curve = fit_result.eval()

    plot_spec = PlotSpec(
        plot_path=str(plot_path),
        title=f"T2* ({plot_type})",
        xlabel="Delay (μs)",
...
def analyze_t2x(loc: Path):

    logger.info("Starting to analyze T2* for {}".format(loc))

    if not loc.exists():
        msg = f"T2* for {loc} does not exist"
        raise FileNotFoundError(msg)

    data = datadict_from_hdf5(loc/"data.ddh5")

    with DatasetAnalysis(loc, "t2x") as ds:
    ...
    for f in savefolders:
        plot_t2x(x=delays, y=signal_re, fit_result=fit_result_re, plot_path=f / "real_fit.png" , plot_type="Real")
        plot_t2x(x=delays, y=signal_imag, fit_result=fit_result_imag, plot_path=f / "imag_fit.png" , plot_type="Imaginary")
        plot_t2x(x=delays, y=signal_mag, fit_result=fit_result_mag, plot_path=f / "mag_fit.png", plot_type="Magnitude")

    return result_re, result_imag, result_mag

def t2x(job_id: str, n_echos: int, fake_calibration_data: bool = False):
    params = get_params()
    old_echo = params.qubit.n_echo()
    # Make sure we return the instrumentserver back to the original state
    try:
        params.qubit.n_echo(n_echos)

        loc, da = measure_t2x(job_id + f"_n_echos={n_echos}", n_echos)
        if fake_calibration_data:
            ...
            # Copy the fake data.ddh5 from FAKEDATAR for 0 echos or FAKEDATAE for the rest to loc
            fake_data_path = FAKEDATAR / "data.ddh5"
            if n_echos > 0:
                fake_data_path = FAKEDATAE / "data.ddh5"
        ...
        ret_real, ret_imag, ret_mag = analyze_t2x(loc)
    finally:
