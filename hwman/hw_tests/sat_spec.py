...
from qcui_measurement.qick.single_transmon_v2 import PulseProbeSpectroscopy

from hwman.utils.fitting import FitSpec, fit_in_subprocess, serialize_params

logger = logging.getLogger(__name__)

FAKEDATA = Path("test_data/2025-09-15T220654_1606b1e8-Saturation spectroscopy~r96bhfg6")


def measure_sat_spec(job_id: str):

    logger.info("Starting saturation spectroscopy for {}".format(job_id))

    sweep = PulseProbeSpectroscopy()
    logger.debug("Sweep created, running measurement")
    loc, da = run_and_save_sweep(sweep, "data", f"saturation_spectroscopy~{job_id}", return_data=True)
...
def plot_sat_spec(x: np.ndarray, y: np.ndarray, fit_result, plot_path: Path, plot_type: str):
    fit_curve = fit_result.eval()

    plot_spec = PlotSpec(
        plot_path=str(plot_path),
        title=f"Saturation Spectroscopy ({plot_type})",
...
def analyze_sat_spec(loc: Path):

    logger.info("Starting to analyze saturation spectroscopy for {}".format(loc))

    if not loc.exists():
        msg = f"Saturation spectroscopy for {loc} does not exist"
        raise FileNotFoundError(msg)

    data = datadict_from_hdf5(loc/"data.ddh5")

    with DatasetAnalysis(loc, "saturation_spectroscopy") as ds:
    ...
    for f in savefolders:
        plot_sat_spec(x=freqs, y=signal_re, fit_result=fit_result_re, plot_path=f / "real_fit.png" , plot_type="Real")
        plot_sat_spec(x=freqs, y=signal_imag, fit_result=fit_result_imag, plot_path=f / "imag_fit.png" , plot_type="Imaginary")
        plot_sat_spec(x=freqs, y=signal_mag, fit_result=fit_result_mag, plot_path=f / "mag_fit.png", plot_type="Magnitude")

    return result_re, result_imag, result_mag

def sat_spec(job_id: str, fake_calibration_data: bool = False):
    loc, da = measure_sat_spec(job_id)
    ...
    ret_real, ret_imag, ret_mag = analyze_sat_spec(loc)
